...
type Fixture = {
  ...
  winner?:string; bye?:boolean; dateTime?:string; stage?:string;
};
type Tournament = {
  name:string; game:string; format:string; players:number; playerNames:string[];
  status:string; date:string; fixtures:Fixture[];
};
...
function PublicShareApp(){
  ...
  return <div className="public-share-shell">
    <header className="public-share-header">
      <div><div className="public-kicker">TGS LIVE</div><h1>{t.name}</h1><p>{t.game} · {t.format} · {t.playerNames?.length||t.players||0} Players</p></div>
      <div className="public-live"><span className="live-dot"/> LIVE <small>Updated {updated}</small></div>
    </header>
    {view==='scorecard' ? <section className="public-card scorecard-view">
      <div className="public-card-title"><div><span>LIVE SCORECARD</span><h2>Standings</h2></div><span>{rows.length} Players</span></div>
      <div className="public-table-wrap"><table className="public-table"><thead><tr><th>#</th><th>PLAYER</th><th>GP</th><th>W</th><th>L</th><th>PTS</th><th>DIFF</th></tr></thead><tbody>{rows.map((r,i)=><tr key={r.name}><td className="rank">{i+1}</td><td className="player">{r.name}</td><td>{r.played}</td><td>{r.wins}</td><td>{r.losses}</td><td className="points">{r.points}</td><td>{r.for-r.against>0?'+':''}{r.for-r.against}</td></tr>)}</tbody></table></div>
    </section> : <section className="public-card fixtures-view">
      <div className="public-card-title"><div><span>LIVE FIXTURES</span><h2>Match Bracket</h2></div><span>{t.fixtures?.length||0} Matches</span></div>
      {!rounds.length?<div className="public-empty">No fixtures have been created yet.</div>:<div className="public-bracket">{rounds.map(r=><div className="public-round" key={r}><div className="public-round-title">{t.fixtures.find(f=>f.roundIndex===r)?.round||`Round ${r+1}`}<small>{t.fixtures.filter(f=>f.roundIndex===r).length} matches</small></div><div className="public-round-matches">{t.fixtures.filter(f=>f.roundIndex===r).map(f=><div className={`public-match ${f.winner?'completed':''}`} key={f.id}><div><b className={f.winner===f.player1?'winner':''}>{f.player1||'TBD'}</b><strong>{f.score1??'—'}</strong></div><div><b className={f.winner===f.player2?'winner':''}>{f.player2||'TBD'}</b><strong>{f.score2??'—'}</strong></div>{f.winner&&<small>{f.bye?'BYE · ':''}{f.winner} advances</small>}</div>)}</div></div>)}</div>}
    </section>}
    <footer className="public-footer"><span>Tech Guru Sumit · TGS Tournament Manager</span><span>{live?'● LIVE SYNC':'Connecting…'}</span></footer>
  </div>;
}

export function mountPublicShare(){
  createRoot(document.getElementById('root')!).render(<React.StrictMode><PublicShareApp/></React.StrictMode>);
}
