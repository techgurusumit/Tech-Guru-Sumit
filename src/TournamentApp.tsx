...
const readImage=(file:File)=>new Promise<HTMLImageElement>((resolve,reject)=>{if(!file.type.startsWith('image/'))return reject(new Error('Please select an image file.'));const reader=new FileReader();reader.onerror=()=>reject(new Error('Could not read the image.'));reader.onload=()=>{const img=new Image();img.onerror=()=>reject(new Error('Could not load the image.'));img.onload=()=>resolve(img);img.src=String(reader.result)};reader.readAsDataURL(file)});
const cropAndResizeImage=(img:HTMLImageElement,zoom:number,posX:number,posY:number,w:number,h:number,quality=.84)=>{const canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;const ctx=canvas.getContext('2d');if(!ctx)throw new Error('Image processing is unavailable.');const base=Math.max(w/img.width,h/img.height);const scale=base*Math.max(.55,zoom);const dw=img.width*scale,dh=img.height*scale;const maxX=Math.max(0,dw-w),maxY=Math.max(0,dh-h);ctx.drawImage(img,(w-dw)/2-(posX/100)*maxX,(h-dh)/2-(posY/100)*maxY,dw,dh);return canvas.toDataURL('image/jpeg',quality)};
function ImageCropModal({editor,close}:{editor:ImageEditorState;close:()=>void}){
 const[img,setImg]=React.useState<HTMLImageElement|null>(null);const[zoom,setZoom]=React.useState(1);const[x,setX]=React.useState(50);const[y,setY]=React.useState(50);
 React.useEffect(()=>{readImage(editor.file).then(setImg).catch(()=>setImg(null))},[editor.file]);
...
function SettingsPage({theme,setTheme,user,logout,updateUser,gamingTheme,setGamingTheme}:{theme:'dark'|'light';setTheme:(x:'dark'|'light')=>void;user:User;logout:()=>void;updateUser:(u:User)=>void;gamingTheme:GamingTheme;setGamingTheme:(x:GamingTheme)=>void}){
 ...
 const upload=(e:React.ChangeEvent<HTMLInputElement>)=>{const file=e.target.files?.[0];e.currentTarget.value='';if(!file)return;setImageEditor({file,title:'Custom Background',shape:'wide',onSave:optimized=>{try{localStorage.setItem('tgs_background',optimized);localStorage.setItem('tgs_background_name',file.name);setBg(optimized);setFileName(file.name);document.documentElement.style.setProperty('--tgs-custom-background','url("'+optimized+'")')}catch{alert('This image is too large. Please choose a smaller image.')}}})};
