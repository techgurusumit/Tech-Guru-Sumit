...
const imageToDataUrl=(file:File)=>new Promise<string>((resolve,reject)=>{const r=new FileReader();r.onerror=()=>reject(new Error('Could not read image.'));r.onload=()=>resolve(String(r.result));r.readAsDataURL(file)});
const readImage=(file:File)=>new Promise<HTMLImageElement>((resolve,reject)=>{imageToDataUrl(file).then(src=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=()=>reject(new Error('Could not load image.'));img.src=src}).catch(reject)});
