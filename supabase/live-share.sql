...
-- The browser never reads this table directly. All access goes through the
-- tightly-scoped RPC functions below.
revoke all on table public.tgs_share_views from anon, authenticated;

after insert or update on public.tgs_share_views
execute function public.tgs_share_views_set_updated_at();
...
-- Recreate the trigger safely after the helper exists.
...
as $$
begin
  if p_view_type not in ('fixtures','scorecard') then
    raise exception 'Invalid view type';
  end if;
  if length(p_public_token) < 32 or length(p_owner_token) < 48 then
  ...
  return query
    ...
    where s.owner_token=p_owner_token;
end;
$$;

create or replace function public.tgs_update_share(
  p_owner_token text,
  p_snapshot jsonb
)
returns timestamptz
...
begin
  ...
  if changed_at is null then
    raise exception 'Invalid share owner token';
  end if;
  return changed_at;
end;
$$;
